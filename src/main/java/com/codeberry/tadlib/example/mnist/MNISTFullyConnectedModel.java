...
public class MNISTFullyConnectedModel implements Model {
    ...
    public MNISTFullyConnectedModel(Factory factory) {
        ...
        hiddenW = tensor(TArrayFactory.randomWeight(weightRnd, shape(IMAGE_SIZE * IMAGE_SIZE, factory.hiddenNeurons)));
        hiddenB = tensor(TArrayFactory.randomWeight(weightRnd, shape(factory.hiddenNeurons)));

        outW = tensor(TArrayFactory.randomWeight(weightRnd, shape(factory.hiddenNeurons, OUTPUTS)));
    ...
    public PredictionAndLosses trainSingleIteration(Random rnd, TrainingData batchData, Optimizer optimizer) {
        ...
        pl.totalLoss.backward(ProviderStore.array(1.0));
