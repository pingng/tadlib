...
public class TADMemory extends Pointer {
     ...
     */
    private static class SharedMemory extends TADMemory {
        private final TADMemory parent;

        public SharedMemory(TADMemory parent, long offset, long size) {
            super();
            this.parent = parent;

            this.bytesAvailable = size;
            this.peer = parent.peer + offset;
        }
        ...
        protected void boundsCheck(long off, long sz) {
            parent.boundsCheck(this.peer - parent.peer + off, sz);
        }

        @Override
        public String toString() {
            return super.toString() + " (shared from " + parent.toString() + ")";
        }
    }
    ...
    public TADMemory(long bytesToAllocate) {
        super(0);
        this.bytesAvailable = bytesToAllocate;
        if (bytesToAllocate <= 0) {
            throw new IllegalArgumentException("Allocation size must be greater than zero");
        }
        peer = malloc(bytesToAllocate);
        if (peer == 0)
            throw new OutOfMemoryError("Cannot allocate " + bytesToAllocate + " bytes");

        this.disposer = new Disposer(peer);

        DisposalRegister.registerDisposer(this, this.disposer);
    }
     ...
     * the original {@link TADMemory} object to avoid GC as long as the shared
    ...
    public Pointer share(long offset, long sz) {
        ...
        return new SharedMemory(this, offset, sz);
    }
     ...
     */
    public TADMemory align(int byteBoundary) {
        if (byteBoundary <= 0) {
        ...
        for (int i = 0; i < 32; i++) {
            if (byteBoundary == (1 << i)) {
                ...
                if ((peer & mask) != peer) {
                    ...
                    long newSize = peer + bytesAvailable - newPeer;
                    if (newSize <= 0) {
                        throw new IllegalArgumentException("Insufficient memory to align to the requested boundary");
                    }
                    return (TADMemory) share(newPeer - peer, newSize);
                }
                return this;
            }
        }
    ...
    public void setString(long offset, String value, String encoding) {
        Charset cs = ensureNonNullCharSet(encoding);

        boundsCheck(offset, value.getBytes(cs).length + 1L);
        super.setString(offset, value, cs.name());
    }

    private static Charset ensureNonNullCharSet(String encoding) {
        try {
    ...
    @Override
    public String toString() {
        return "allocated@0x" + Long.toHexString(peer) + " (" + bytesAvailable + " bytes)";
    }
