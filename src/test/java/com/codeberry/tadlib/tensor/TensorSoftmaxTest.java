...
public class TensorSoftmaxTest {
    @Test
    public void softmax() {
        Tensor input = new Tensor(new TArray(new double[]{0.234, 2.73, -5.3, 2, 2.92, 0.2})
                .reshape(1, -1));
        Tensor labels = new Tensor(new TArray(new double[]{1., 0., 0., 0, 0, 0})
                .reshape(1, -1));

        TArray softmax = input.vals.softmax();
        ...
        cost.backward(new TArray(backpropGrad));
