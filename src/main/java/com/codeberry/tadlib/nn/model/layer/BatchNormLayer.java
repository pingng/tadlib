...
public class BatchNormLayer implements Layer {
    ...
    public OpsExtended.BatchNormRunningAverages runningAverages = new OpsExtended.BatchNormRunningAverages();

    private final Shape outputShape;

    public BatchNormLayer(Random rnd, Shape inputShape, Builder params) {
        int paramsLen = OpsExtended.guessParamLength(inputShape);
    ...
    public ForwardResult forward(Random rnd, Tensor inputs, RunMode runMode) {
        OpsExtended.BatchNormResult result = OpsExtended.batchNorm(inputs, beta, gamma, runningAverages, runMode);
