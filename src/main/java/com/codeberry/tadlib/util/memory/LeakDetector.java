...
public class LeakDetector {
    ...
    private static class AllocationInfo {
        ...
        private volatile int allocationCount;
        private volatile int releaseCount;
        private boolean printOnRelease;

        AllocationInfo(long objectId, AtomicLong objectYear) {
        ...
        synchronized void addAllocation(StackTraceInfo traceInfo) {
            allocStackTraces.add(traceInfo);
            allocationCount++;
        }

        synchronized void addRelease(StackTraceInfo traceInfo, Runnable onFullyReleased) {
        ...
        synchronized void enableReleasePrint() {
